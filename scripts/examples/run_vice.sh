softlearning run_example_local examples.classifier_rl \
        --exp-name=vice_final `# This is the name your experiment will be saved under ~/ray_results` \
        --algorithm=VICE  `# Algorithm of choice (ex: SAC, VICE, etc.)` \
        --num-samples=8  `# Number of seeds PER choice of hyperparameters (will be multiplied by number of param combinations if you tune over a bunch)` \
