softlearning run_example_local examples.classifier_rl \
        --exp-name=vice_online_gt_count_reproduce `# This is the name your experiment will be saved under ~/ray_results` \
        --algorithm=DynamicsAwareEmbeddingVICE  `# Algorithm of choice (ex: SAC, VICE, etc.)` \
        ...
        --trial-gpus=0.25  `# Number of GPUs that will be utilized PER seed` \
        ...
        --n-epochs=500  `# Num of epochs to run for` \
        --n-goal-examples=200  `# Number of goal examples to collect for VICE` \
