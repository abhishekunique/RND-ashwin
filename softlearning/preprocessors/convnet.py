...
def hacky_two_image_convnet_preprocessor(
        input_shapes,
        image_shape,
        output_size,
        conv_filters=(32, 32),
    ...
    image_flat_size = H * W * C
    images_1_flat, images_2_flat, input_raw = tf.keras.layers.Lambda(
        lambda x: [
            x[..., :image_flat_size],
            x[..., image_flat_size:2*image_flat_size],
            x[..., 2*image_flat_size:]
        ])(concatenated_input)

    images_1 = tf.keras.layers.Reshape(image_shape)(images_1_flat)
    images_2 = tf.keras.layers.Reshape(image_shape)(images_2_flat)

    image_input = tf.keras.layers.Input(shape=image_shape)
    conv_out = image_input
    for filters, kernel_size, pool_size, strides in zip(
            conv_filters, conv_kernel_sizes, pool_sizes, pool_strides):
        conv_out = tf.keras.layers.Conv2D(
            filters=filters,
    ...
    convnet = tf.keras.Model(image_input, conv_out, name='convnet')

    preprocessed_images = []
    for images in (images_1, images_2):
        conv_out = convnet(images)
        flattened = tf.keras.layers.Flatten()(conv_out)
        preprocessed_images += [flattened]
